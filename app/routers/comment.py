...
async def create_new_comment(comment: schemas.CommentCreate, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    new_comment = models.Comments(user_id=current_user.UId, **comment.dict())
    db.add(new_comment)
    db.commit()
...
async def get_all_comments(db: Session = Depends(get_db), limit: int = 10, skip: int = 0, search: Optional[str] = ''):

    # comments = db.query(models.Comments).all()
    comments = db.query(models.Comments).filter(models.Comments.CommentDetail.contains(search)).order_by('CoId').limit(limit).offset(skip).all()
    return comments


@router.get('/getcommentbyqueryid/{query_id}')
async def get_comment_by_qid(query_id: int, db: Session = Depends(get_db)):
    get_all_comments_by_qid = db.query(models.Comments).filter(
        models.Comments.query_id == query_id).all()

    if not get_all_comments:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f'${query_id} comments not found')
...
async def update_comment(comment: schemas.CommentCreate, comment_id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    print(comment)
    get_single_comment = db.query(models.Comments).filter(
        models.Comments.CoId == comment_id)

    if get_single_comment.first() == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f'${comment_id} not found')

    if get_single_comment.first().user_id != current_user.UId:
